...
/**
 * J2StoreCategories Class.
 */
class J2StoreCategories
{
	/**
	...
	public static function getInstance($extension, $options = array())
	{
		...
		{
			$path = JPATH_SITE . '/modules/mod_j2store_categories/helpers/categories.php';
			if (is_file($path))
			{
				include_once $path;
			}
	...
	protected function _load($id)
	{
		...
		if (isset($this->_options['countItems']) && $this->_options['countItems'] == 1)
		{
			if ($this->_options['published'] == 1)
			{
				...
				$query->join('INNER', '#__j2store_products ON #__j2store_products.product_source='.$db->q('com_content').' AND #__j2store_products.product_source_id = i.id');
 ...
 * @since       11.1
 */
class J2StoreCategoryNode extends JObject
{
	/**
	 * Primary key
	 *
